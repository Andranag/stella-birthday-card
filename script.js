...
function startSearchSpawn(slide) {
    ...
    const tick = () => {
        if (!slide.isConnected) { clearSearchStamps(); return }
        if (i >= 6) { clearInterval(searchSpawnIntervalId); searchSpawnIntervalId = null; return }
        ...
        stamp.style[alignRight ? 'marginRight' : 'marginLeft'] = `${Math.round(Math.random() * 56)}px`
...
function clearAgainStamps() {
    document.getElementById('kiss-again-slide')?.querySelector('.again-stamps')?.remove()
}
...
function stopAgainFill() {
    ...
    if (slide) { slide.dataset.againArmed = 'false'; slide.dataset.againFilled = 'false' }
    clearAgainStamps()
}
...
function startAgainFill(slide) {
    if (!slide || slide.dataset.againArmed !== 'true' || slide.dataset.againFilled === 'true') return
    ...
    let i = 0
    const tick = () => {
        if (!slide.isConnected) { clearAgainStamps(); return }
        if (i >= 52) { clearInterval(againFillIntervalId); againFillIntervalId = null; return }
...
function getOrCreateTypingState(slide) {
    if (!slideTypingState.has(slide)) {
        slideTypingState.set(slide, { timeouts: [], isTyping: false, plan: null, index: 0, danceIndex: 0 })
    }
...
function isTypingArmedForSlide(slide) { return slide?.dataset.twArmed === 'true' }
...
function resetDanceSkillsText(slide) {
    ...
    state.danceIndex = 0
    Array.from(slide.querySelectorAll('.dance-text .gift-title')).forEach(el => {
        if (el.dataset.twFullText == null) el.dataset.twFullText = el.textContent || ''
        if (!prefersReducedMotion) {
            const rect = el.getBoundingClientRect()
            if (rect.height > 0) { el.dataset.twMinHeight = String(rect.height); el.style.minHeight = `${rect.height}px` }
            el.textContent = ''
        }
    })
}
...
function typeTextIntoElement(el, fullText, state, onDone) {
    if (prefersReducedMotion) { el.textContent = fullText; onDone(); return }
    el.textContent = ''
    let i = 0
    const step = () => {
        ...
        if (i >= fullText.length) {
            const id = setTimeout(() => {
                ...
                if (slide?.id === 'kiss-again-slide' && el === slide.querySelector('h2.gift-title')) {
                    if (isGiftRevealed(slide.querySelector('#gift-img-kiss6')) && isGiftRevealed(slide.querySelector('#gift-img-kiss7'))) {
                        startAgainFill(slide)
                    }
                }
                if (slide && el === slide.querySelector('h2.gift-title') && slide.querySelector('#search-spawn') && searchSpawnIntervalId == null) {
        ...
        const id = setTimeout(step, fullText[i - 1] === ' ' ? TYPING.spaceMs : TYPING.charMs)
        state.timeouts.push(id)
    }
    step()
}
...
function runTypingAdvance(slide, elements, state) {
    const advance = () => {
        if (!state.isTyping) return
        const next = elements.find(it => !isTypingElementDone(it.el) && (!it.requiredGift || isGiftRevealed(it.requiredGift)))
        if (!next) { state.isTyping = false; return }
...
function startStoryTypingForSlide(slide) {
    ...
    const state = getOrCreateTypingState(slide)
    state.isTyping = true
    state.plan = buildSlideTypingPlan(slide).filter(item =>
        (item.el.dataset.twFullText ?? item.el.textContent ?? '').trim().length > 0
    )
    if (prefersReducedMotion) {
        state.plan.forEach(({ el }) => { el.textContent = el.dataset.twFullText ?? el.textContent })
        return
    }
...
function maybeTypeNextDanceLine(slide) {
    ...
    typeTextIntoElement(el, fullText, state, () => {
        ...
        state.isTyping = false
        updateScrollableSlides()
        state.timeouts.push(setTimeout(() => maybeTypeNextDanceLine(slide), TYPING.lineGapMs))
    })
}
...
function renderQuizStep() {
    ...
    if (quizAnswer) { quizAnswer.value = ''; quizAnswer.focus() }
}
...
function isUnlocked() {
    try { return window.localStorage.getItem(QUIZ_STORAGE_KEY) === 'true' } catch { return false }
}

function unlockQuizGate() {
    try { window.localStorage.setItem(QUIZ_STORAGE_KEY, 'true') } catch { /* ignore */ }
...
function setCacheBustedRevealImage(gift) {
    ...
    if (!match) return
    gift.style.setProperty('--reveal-image', `url("${match[2].split('?')[0]}?v=${Date.now()}")`)
}
...
function toggleGiftReveal(gift) {
    ...
    if (revealing) {
        ...
        gift.style.setProperty('--gift-rot', `${(-2.5 + Math.random() * 5).toFixed(2)}deg`)
    } else {
    ...
    if (revealing) {
        ...
        if (slide?.id === 'kiss-again-slide') {
            ...
            if (isTypingElementDone(triggerEl) &&
                isGiftRevealed(slide.querySelector('#gift-img-kiss6')) &&
                isGiftRevealed(slide.querySelector('#gift-img-kiss7'))) {
                startAgainFill(slide)
            }
        }
        if (slide && isDanceSkillsSlide(slide)) { maybeTypeNextDanceLine(slide); return }
            ...
            if (slide) {
                if (isDanceSkillsSlide(slide)) {
                    const ordered = ['#gift-img-dance-skills', '#gift-img-dance-skills2', '#gift-img-dance-skills3']
                        .map(sel => slide.querySelector(sel)).filter(Boolean)
                    if (ordered.length > 1) {
                        const idx = ordered.indexOf(gift)
                        if (idx > 0 && !ordered[idx - 1]?.classList.contains('revealed')) return
                    }
                }

                const siblings = Array.from(slide.children).filter(el => el.classList?.contains('gift-img') && el.classList.contains('tap-reveal'))
                if (siblings.length > 1) {
                    const idx = siblings.indexOf(gift)
                    if (idx > 0 && !siblings[idx - 1]?.classList.contains('revealed')) return
                }
            }
        }
...
function setActiveSlide(index) {
    ...
    if (activeSlideIndex >= 0) {
        ...
        if (prev) { clearSlideTyping(prev); resetStoryElements(prev); disarmTypingForSlide(prev) }
        stopSearchSpawn()
        stopAgainFill()
    }
...
function updateScrollableSlides(fullRecompute = false) {
    const targets = fullRecompute ? slides : [slides[activeSlideIndex]].filter(Boolean)
    targets.forEach(slide => {
        slide.style.removeProperty('--nav-safe-pad')
        slide.classList.toggle('is-scrollable', slide.scrollHeight > slide.clientHeight)
    })
}
...
function goToIndex(index) {
    ...
    const pageWidth = swipeContainer.clientWidth || window.innerWidth
    swipeContainer.scrollTo({ left: Math.max(0, Math.min(index, slides.length - 1)) * pageWidth, behavior: 'smooth' })
}
...
swipeContainer?.addEventListener('scroll', () => {
    ...
    navRafPending = true
    requestAnimationFrame(() => { navRafPending = false; updateNav() })
}, { passive: true })

window.addEventListener('resize', () => { updateScrollableSlides(true); updateNav() })
...
function autoGateStoryTextToNextGift() {
    slides.forEach(slide => {
        ...
        nodes.forEach((node, idx) => {
            ...
            if (node.classList.contains('gift-img') || node.classList.contains('typewriter')) return
...
function wrapGiftSectionText() {
    document.querySelectorAll('.gift-section').forEach(section => {
        const directGifts = Array.from(section.children).filter(el => el.classList?.contains('gift-img'))
        if (section.querySelector(':scope > .gift-text')) return

        if (directGifts.length === 1) {
            const wrapper = document.createElement('div')
            wrapper.className = 'gift-text'
            Array.from(section.children).forEach(child => { if (child !== directGifts[0]) wrapper.appendChild(child) })
            section.appendChild(wrapper)
        } else if (directGifts.length === 2) {
            if (section.id === 'first-date-lift-slide') return
            if (section.querySelector(':scope > .dance-collage, :scope > .search-scene, :scope > .float-text')) return

            const wrapper = document.createElement('div')
            wrapper.className = 'gift-text'
            Array.from(section.children).forEach(child => {
                if (child !== directGifts[0] && child !== directGifts[1]) wrapper.appendChild(child)
            })
...
wrapGiftSectionText()
document.querySelectorAll('.gift-hint').forEach(h => h.classList.remove('is-hidden'))
document.querySelectorAll('.gift-img.revealed').forEach(gift => fitGiftToRevealImage(gift))
